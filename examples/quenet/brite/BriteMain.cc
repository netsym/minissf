...
public:
  ...
  void InitSeeds(int);
#else
  void InitSeeds(const char *);
#endif
  void OutputSeeds(const char *);
...
#define RND_MULTIPLIER 48271
#define RND_Q (RND_MODULUS/RND_MULTIPLIER)
#define RND_R (RND_MODULUS%RND_MULTIPLIER)
static int seed;
static unsigned short int RAND() {
  int t = RND_MULTIPLIER*(seed%RND_Q)-RND_R*(seed/RND_Q);
  if(t > 0) seed = t;
  else seed = t + RND_MODULUS;
  return (unsigned short int)seed;
}
static void SETSEED(unsigned short int seed[]) {
  seed[0] = RAND();
  seed[1] = RAND();
  seed[2] = RAND();
}
...
void BriteMain::InitSeeds(int init_seed) {
  if(init_seed) seed = init_seed;
  else seed = MAKESEED();
  SETSEED(Model::s_places);
  SETSEED(Model::s_connect);
  SETSEED(Model::s_edgeconn);
  SETSEED(Model::s_grouping);
  SETSEED(Model::s_assignment);
  SETSEED(Model::s_bandwidth);
}
#else
void BriteMain::InitSeeds(const char *file_name) {
...
void BriteMain::OutputSeeds(const char *file_name) {
...
#ifdef QUENET
  const char* config_filename = "default.conf";
  const char* output_filename = "model.quenet";
  int init_seed = 0;
  if(argc == 1) {}
  else if(argc == 2) { config_filename = argv[1]; }
  else if(argc == 3) { config_filename = argv[1]; output_filename = argv[2]; }
  else if(argc == 4) { config_filename = argv[1]; output_filename = argv[2]; init_seed = atoi(argv[3]); }
  else {
    cerr << argv[0] << " [config-file [output-file [random-seed]]]\n";
    exit(0);
  }
  cout << "Usage: " << argv[0] << " [config-file [output-file [random-seed]]]\n" <<
...
#ifdef QUENET
    topology->QuenetOutput(output_filename);
#else
  if (p.OutputBrite()) {
