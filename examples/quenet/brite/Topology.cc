...
#ifdef QUENET
void Topology::QuenetOutput(const char* filename) 
{
  ofstream outfile;
  ...
  outfile << g->GetNumNodes() << " 10\n";

  for (int i = 0; i < g->GetNumNodes(); i++) {
    int myid = g->GetNodePtr(i)->GetId();
    int fanout = g->GetNodePtr(i)->GetOutDegree();
    outfile << myid << " FIFO 1.0 " << fanout;
    list<Edge*>::iterator el;
    double bw = 0;
    for(el = g->edges.begin(); el != g->edges.end(); el++) {
      if(((*el)->GetSrc()->GetId() == myid) ||
	 ((*el)->GetDst()->GetId() == myid)) 
	bw += (*el)->GetConf()->GetBW();
    }
    for(el = g->edges.begin(); el != g->edges.end(); el++) {
      if((*el)->GetSrc()->GetId() == myid) {
	outfile << " " << (*el)->GetDst()->GetId() << " " 
		<< ((bw>0)?((*el)->GetConf()->GetBW()/bw):(1.0/fanout)) << " " << (*el)->Length();
      } else if((*el)->GetDst()->GetId() == myid) {
	outfile << " " << (*el)->GetSrc()->GetId() << " " 
		<< ((bw>0)?((*el)->GetConf()->GetBW()/bw):(1.0/fanout)) << " " << (*el)->Length();
      }
    }
    outfile << "\n";
...
void Topology::BriteOutput(const char* filename) {
...
void Topology::OtterOutput(const char* filename) {
