/*
 * The code is modified from RNG by Pierre L'Ecuyer.
 * It is a CMRGs generator which has 2 components each of order 3.
 * At each step n its state is the pair of vectors:
 * s(1,n) =(x(1,n), x(1,n+1), x(1,n+2)), s(2,n) =(x(2,n), x(2,n+1), x(2,n+2)),
 * which evolve according to the linear recurrences:
 * x(1,n) = (a12 * x(1,n−2) − a13 * x(1,n−3)) mod m1
 * x(2,n) = (a21 * x(2,n−1) − a23 * x(2,n−3)) mod m2
 * output is defined by
 * zn = (x(1,n) - x(2,n) mod 4294967087)
 * u = zn / 4294967088, if zn > 0
...
// declared LecuyerRandom, unless SetPackageSeed is called.
//
double LecuyerRandom::nextSeed[6] =
  {
...
// constructor
//
LecuyerRandom::LecuyerRandom (int seed)
{
  anti = false;
  incPrec = false;
  ...
  /* Information on a stream. The arrays {Cg, Bg, Ig} contain the current
     ...
     will be the seed of the next declared LecuyerRandom. */
  ...
  if (seed >= 0){
    ...
    for (i = 0; i < 3; i++) {
      nextSeed[i] = ((double)rand() / RAND_MAX) * m1 + 1.0;
    }

    for (i = 3; i < 6; i++) {
      nextSeed[i] = ((double)rand() / RAND_MAX) * m2 + 1.0;
    }
  }
...
//
void LecuyerRandom::ResetStartStream ()
{
  for (int i = 0; i < 6; ++i)
    Cg[i] = Bg[i] = Ig[i];
}
...
//
void LecuyerRandom::ResetStartSubstream ()
{
  for (int i = 0; i < 6; ++i)
    Cg[i] = Bg[i];
}
...
//
void LecuyerRandom::ResetNextSubstream ()
{
...
//
void LecuyerRandom::AdvanceState (long e, long c)
{
...
//-------------------------------------------------------------------------
void LecuyerRandom::GetState (unsigned long seed[6]) const
{
  for (int i = 0; i < 6; ++i)
...
//-------------------------------------------------------------------------
void LecuyerRandom::WriteState () const
{
...
//-------------------------------------------------------------------------
void LecuyerRandom::WriteStateFull () const
{
  int i;

  cout << "The LecuyerRandom";
...
//-------------------------------------------------------------------------
void LecuyerRandom::IncreasedPrecis (bool incp)
{
  incPrec = incp;
}
...
//-------------------------------------------------------------------------
void LecuyerRandom::SetAntithetic (bool a)
{
  anti = a;
}
...
//
int LecuyerRandom::RandInt (int low, int high)
{
  return low + static_cast<int> ((high - low + 1.0) * RandU01 ());
}

//-------------------------------------------------------------------------
void LecuyerRandom::setSeed(int s)
{
  int tmp = rand();

  if (s == 0) srand (make_new_seed());
  else srand(s);

  int seed[6], i;
  for (i = 0; i < 3; i++) {
    seed[i] = ((double)rand() / RAND_MAX) * m1 + 1.0;
  }

  for (i = 3; i < 6; i++) {
    seed[i] = ((double)rand() / RAND_MAX) * m2 + 1.0;
  }
